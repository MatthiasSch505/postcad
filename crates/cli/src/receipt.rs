...
/// Canonical routing input envelope committed at routing time.
///
/// Captures every input that determines a routing decision. Serialized to
/// canonical JSON (alphabetically sorted keys, compact) and hashed as
/// [`RoutingReceipt::routing_input_hash`], binding the receipt to the exact
/// routing inputs rather than only to derived per-field fingerprints.
///
/// Stored inline in [`RoutingReceipt`] so verifiers can recompute
/// `routing_input_hash` without access to the original input files.
///
/// Field name mapping from the external domain vocabulary to PostCAD:
/// - `procedure`        — the dental procedure type; analogous to "device type"
/// - `patient_country`  — the patient's (clinic) country; analogous to "clinic country"
/// - `manufacturer_country` — the manufacturing country
/// - `file_type`        — the dental file format (stl, obj, …)
/// - `jurisdiction`     — the routing jurisdiction (e.g. "DE", "global")
/// - `routing_policy`   — the routing policy variant string
///
/// `timestamp` and `requested_turnaround` are intentionally excluded:
/// timestamps are non-deterministic, and `requested_turnaround` is not part
/// of the PostCAD domain.
#[derive(Debug, Serialize, Deserialize, PartialEq)]
pub struct RoutingInputEnvelope {
    pub case_id: String,
...
pub struct RoutingReceipt {
    ...
    /// Routing kernel algorithm identifier. Always `"postcad-routing-v1"` for
    /// receipts generated by the current pipeline. Verified during
    /// `verify-receipt`; a mismatch indicates the receipt was produced by a
    /// different routing algorithm and cannot be validated against the current
    /// kernel.
    ...
    /// Full routing input envelope — all inputs that determined this decision.
    ///
    /// Stored inline so that `routing_input_hash` can be recomputed by any
    /// verifier without access to the original `case.json` / `policy.json`
    /// files. Mutating any field here and recomputing `receipt_hash` without
    /// also updating `routing_input_hash` is detectable during `verify-receipt`.
    pub routing_input: RoutingInputEnvelope,
    /// SHA-256 of the canonical routing input envelope.
    ///
    /// Computed as `SHA-256(canonical_json(routing_input))` where
    /// `canonical_json` serializes fields in alphabetical key order (compact,
    /// no whitespace). Included in [`receipt_hash`], so tampering with either
    /// `routing_input` or `routing_input_hash` invalidates the receipt.
