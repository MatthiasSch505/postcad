...
if [[ "${1:-}" == "--pilot-demo" ]]; then
  ...
  echo ""
  echo "--------------------------------------"
  echo "Copy this to share PostCAD:"
  echo "--------------------------------------"
  echo ""
  echo "PostCAD = deterministic routing + compliance + audit layer after CAD."
  echo ""
  echo "CAD"
  echo "↓"
  echo "PostCAD routing"
  ...
  echo ""
  echo "Properties:"
  echo "- no clinical decision making"
  echo "- no manufacturing liability"
  echo "- fully auditable"
  echo "- jurisdiction-aware routing"
  echo ""
  exit 0
fi
...
if [[ "${1:-}" == "--help-surface" ]]; then
  ...
  echo ""
  echo "--operator-cheatsheet"
  echo "  ./examples/pilot/run_pilot.sh --operator-cheatsheet"
  echo "  Purpose : Print a compact one-screen operator reference with sections:"
  echo "            WHAT THIS SHELL DOES, CORE COMMANDS, WHEN TO USE THEM, SAFE REVIEW PATH."
  echo "  Use when: you want a fast, deterministic overview of all core commands."
  echo ""
  echo "────────────────────────────────────────"
  echo "Recommended order"
  echo ""
...
# ── Mode: operator cheatsheet ─────────────────────────────────────────────────

if [[ "${1:-}" == "--operator-cheatsheet" ]]; then
  echo ""
  echo "PostCAD Pilot — Operator Cheatsheet"
  echo "════════════════════════════════════════════════════════════"
  echo ""
  echo "WHAT THIS SHELL DOES"
  echo "  run_pilot.sh is the deterministic pilot operator shell for PostCAD."
  echo "  It routes dental cases against the manufacturer registry, writes a"
  echo "  cryptographic receipt, and provides inspection, verification, and"
  echo "  dispatch surfaces. Every output is deterministic and auditable."
  echo "  No network calls. No AI. No timestamps in static output."
  echo ""
  echo "CORE COMMANDS"
  echo "  ./examples/pilot/run_pilot.sh"
  echo "    Generate pilot bundle — route case, write receipt.json."
  echo ""
  echo "  ./examples/pilot/run_pilot.sh --inspect-inbound-reply <file>"
  echo "    Inspect inbound lab reply — check required fields before verification."
  echo ""
  echo "  ./examples/pilot/run_pilot.sh --export-dispatch"
  echo "    Export dispatch packet — confirm dispatch status and show next action."
  echo ""
  echo "  ./examples/pilot/run_pilot.sh --artifact-index"
  echo "    Artifact index — print the file map for the current run."
  echo ""
  echo "  ./examples/pilot/run_pilot.sh --run-summary"
  echo "    Run summary — human-readable overview of the current run state."
  echo ""
  echo "  ./examples/pilot/run_pilot.sh --walkthrough"
  echo "    Walkthrough — full 4-step pilot workflow guide."
  echo ""
  ...
  echo ""
  echo "  ./examples/pilot/run_pilot.sh --help-surface"
  echo "    Help surface — full operator mode reference with recommended order."
  echo ""
  echo "  ./examples/pilot/run_pilot.sh --operator-cheatsheet"
  echo "    Operator cheatsheet — this screen."
  echo ""
  echo "  ./examples/pilot/verify.sh --inbound <file> --bundle examples/pilot"
  echo "    Verify inbound reply — cryptographic verification against receipt."
  echo ""
  echo "WHEN TO USE THEM"
  echo "  Starting a new pilot run?          (default — no argument)"
  echo "  Lab reply arrived?                 --inspect-inbound-reply <file>"
  echo "  Ready to cryptographically verify? verify.sh --inbound <file> --bundle ..."
  echo "  Checking dispatch readiness?       --export-dispatch"
  echo "  Need all file locations?           --artifact-index"
  echo "  Need a full run overview?          --run-summary"
  echo "  First time learning the workflow?  --walkthrough"
  echo "  Need just the key commands fast?   --quickstart"
  echo "  Need all modes listed?             --help-surface"
  echo ""
  echo "SAFE REVIEW PATH"
  echo "  Recommended deterministic sequence for a first operator pass:"
  echo ""
  echo "  1. Generate pilot bundle"
  echo "       ./examples/pilot/run_pilot.sh"
  echo ""
  echo "  2. Confirm artifact locations"
  echo "       ./examples/pilot/run_pilot.sh --artifact-index"
  echo ""
  echo "  3. Review run state"
  echo "       ./examples/pilot/run_pilot.sh --run-summary"
  echo ""
  echo "  4. Inspect inbound lab reply (once received)"
  echo "       ./examples/pilot/run_pilot.sh --inspect-inbound-reply inbound/lab_reply_<run-id>.json"
  echo ""
  echo "  5. Verify inbound reply"
  echo "       ./examples/pilot/verify.sh --inbound inbound/lab_reply_<run-id>.json --bundle examples/pilot"
  echo ""
  echo "  6. Confirm dispatch readiness"
  echo "       ./examples/pilot/run_pilot.sh --export-dispatch"
  echo ""
  echo "════════════════════════════════════════════════════════════"
  echo ""
